...
> AES-256 encrypt/decrypt a file with a passphrase
...
Python 3.6+ and openssl CLI

## Usage

```bash
file-crypt <enc|dec> <infile> <outfile>
```

## Examples

```bash
file-crypt enc secret.txt secret.enc
```
```bash
file-crypt dec secret.enc secret.txt
```
